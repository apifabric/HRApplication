...
resourcename="hrapplication"  # lower case, only
resourcegroup="hrapplication_rg"
...
# git clone https://github.com/apilogicserver/hrapplication.git
# cd hrapplication
...
az group create --name hrapplication_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group hrapplication_rg --sku S1 --is-linux
az container create --resource-group hrapplication_rg --name hrapplication --image apilogicserver/hrapplication:latest --dns-name-label hrapplication --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//hrapplication.westus.azurecontainer.io:5656'
